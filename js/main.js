...
var Board = (function () {
      ...
      for (var i = -1; i < 2; i += 1) {
        for (var j = -1; j < 2; j += 1) {
          ...
          if (x + i >= 0 && x + i < width && y + j >= 0 && y + j < height) {
            ...
            // empty tile
            if (
              currentTile.numberOfAdjacentMines == 0 &&
              Math.abs(i + j) == 1 &&
  ...
  Board.prototype.recursiveReveal = function (tilesToClear) {
    ...
    if (tilesToClear.length == 0) {
      ...
      arr = self
        .recursiveReveal([first])
        .concat(self.recursiveReveal(tilesToClear));
      returned = arr;
    }
...
var Game = (function () {
  ...
  Game.prototype.gameOver = function (won) {
    ...
    if (won) {
      self.drawGUI(
        "Congratulations! Score: " + self.time + "s. Click to play again"
      );
    } else {
